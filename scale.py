...
scale = ['C3','D3','E3','F3','G3','A3','B3','C4','D4','E4','F4','G4','A4','B4','C5','D5']
note_values = [0.5]*len(scale)
factor = [0.68, 0.26, 0.03, 0.  , 0.03]
length = [0.01, 0.6, 0.29, 0.1]
decay = [0.05, 0.02, 0.05, 0.1]
sustain_level = 0.7
...
scale_sustain = utils.get_song_data(scale, note_values, bar_value=8,
                                 factor=factor, length=length, decay=decay, 
                                 sustain_level=sustain_level)

scale_sustain = scale_sustain * (4096/np.max(scale_sustain))

print(scale_sustain, flush=True)
